...
type ToolKey = 'seedance' | 'json' | 'timestamp' | 'aiconvert' | 'llmbatch' | 'imgtest' | 'modelprobe' | 'imganalyze'
...
const TOOLS: { key: ToolKey; label: string; icon: React.ReactNode }[] = [
  ...
  { key: 'imgtest', label: '图片接口测试', icon: <IconImgTest /> },
...
function IconGraphql() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
      <path d="M5 8.6a3.4 3.4 0 0 0 1.7 6.3c.6 0 1.2-.2 1.7-.5l6 3.2a3.4 3.4 0 0 0-.4 2.9 3.4 3.4 0 1 0 1.8-1.1l-6-3.3a3.4 3.4 0 0 0 0-3.9l6-3.2a3.4 3.4 0 0 0 3-5.2 3.4 3.4 0 1 0-4.4 1.3l-6 3.3a3.4 3.4 0 0 0-4-1z"/>
    </svg>
  )
}
function IconImgTest() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
      <rect x="3" y="4" width="18" height="16" rx="3"/><circle cx="8.5" cy="9.5" r="1.6"/><path d="m4 17 4.5-4.5 3 3L15 11l5 5"/>
...
interface ImgParsedResponse {
  ...
  httpStatus: number
  headers: Record<string, string>
  images: { url: string | null; dataUri: string | null }[]
  rawSnippet: string
  error: string | null
}
function imgParseResponse(type: ImgApiType, text: string, headers: Record<string, string>, httpStatus: number, ok: boolean, fmtHint: string | undefined): ImgParsedResponse {
  const out: ImgParsedResponse = { ok, httpStatus, headers, images: [], rawSnippet: text.slice(0, 4000), error: null }
  let json: any = null
  ...
  if (type === 'gemini') {
    ;(json.candidates || []).forEach((c: any) => (c.content?.parts || []).forEach((p: any) => {
      ...
      if (d && d.data) { const mime = (d.mimeType || d.mime_type || 'image/png'); out.images.push({ dataUri: imgB64ToDataURI(d.data, mime.split('/')[1]), url: null }) }
    }))
  } else {
    const arr = json.data || []
    out.images = arr.map((d: any) => ({ url: d.url || null, dataUri: d.b64_json ? imgB64ToDataURI(d.b64_json, fmtHint) : null }))
  }
...
function ImgApiTestTool() {
  ...
  const runCase = async (c: ImgCase) => {
    ...
    const rec: ImgRecord = {
      ...
      refThumbs: [],
      status: 0, respHeaders: {}, reqId: '', sentPreview: '',
      ok: false, error: null, rawSnippet: '', images: [], returnedN: 0, durationMs: 0, checks: [],
    }
    try {
      ...
      if (parsed.ok) {
        ...
        for (const im of parsed.images) {
          ...
          const dim = dataUri ? await imgProbeImage(dataUri) : { w: 0, h: 0 }
          const thumb = await imgMakeThumb(dataUri)
          imgs.push({ dataUri, thumb, url: im.url || null, w: dim.w, h: dim.h, format: imgDetectUriFormat(dataUri) })
        }
        rec.images = imgs
    ...
    const histRec: ImgRecord = { ...rec, images: rec.images.map(im => ({ ...im, dataUri: null })) }
  ...
  const renderResultBody = (r: ImgRecord) => (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        {[['响应头', JSON.stringify(r.respHeaders || {}, null, 2)], ['响应体（截断）', r.rawSnippet || ''], ['已发送的请求体（占位符已替换 · base64 已省略）', r.sentPreview || '']].map(([label, body]) => (
          <details key={label} className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
            ...
            <pre className="p-3 text-[11px] font-mono overflow-auto max-h-60 whitespace-pre-wrap break-all leading-relaxed" style={{ color: 'var(--t2)' }}>{imgEsc(body)}</pre>
          </details>
        ))}
      </div>
    </div>
    ...
    <div className="h-full flex" style={{ background: 'transparent' }}>
      ...
      {detailRec && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto p-8" style={{ background: 'rgba(0,0,0,0.4)' }} onClick={() => setDetailRec(null)}>
          <div className="w-full max-w-3xl rounded-2xl ia-card-enter" style={{ background: 'var(--bg)', boxShadow: 'var(--shadowMd)', border: '1px solid var(--border)', maxHeight: '82vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
...
const FULLHEIGHT_TOOLS: ToolKey[] = ['json', 'aiconvert', 'llmbatch', 'imgtest', 'modelprobe', 'base64', 'unicode', 'graphql']
